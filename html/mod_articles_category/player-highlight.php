...
<div class="view-listings-wrap list-course style-<?php echo $modStyle; ?>">
	<div class="row row-cols-1 row-cols-sm-2 row-cols-lg-6 v-gutters">
      ...
      $imgexists =
          (isset($images->image_intro) and !empty($images->image_intro)) ||
          (isset($images->image_fulltext) and !empty($images->image_fulltext));

      if ($imgexists) {
          $images->image_intro = $images->image_intro
              ? $images->image_intro
            ...
            $images->image_intro
							...
							<?php if (!empty($customs['team-role'])): ?>
				        <div class="team-role">
				          <span class="<?php echo implode(
                  ',',
                  $customs['team-role']
              ); ?>"></span>
				        </div>
      ...
      ?>
						...
						<div class="intro-wrap">
				          ...
				          <div class="fs-xs mb-1 stt-time stt-<?php echo strtolower(
                  preg_replace('/\s*/', '', $ltStatus)
              ); ?>"><?php echo $ltStatus; ?></div>
				        <?php endif; ?>

                        <?php if (!empty($customs['player-position'])): ?>
				          <div class="player-highlight-team fs-xs">
				            <?php echo implode(',', $customs['player-position']); ?>,
				          </div>
				        <?php endif; ?> 
				        <?php if ($playerCurrentTeam): ?>
				          <div class="player-highlight-team fs-xs"><b><?php echo $playerCurrentTeam; ?></b></div>
				        <?php endif; ?>
              ...
              $item->tags->itemTags
                ...
                $params->get('readmore_limit')
							...
							<?php  ?>
							<div class="bottom-info <?php if (!empty($item->rating_count)) {
           echo 'has-jrate';
       } ?>">
								<!-- Show voting form -->
               ...
               ?> 
					            ...
					            <div class="number-rating"><?php echo $rateCount .
                     ' ' .
                     Jtext::_('TPL_REVIEWS'); ?></div>
					        </div>
								  <!-- End showing -->
								<?php endif; ?>
	...
	<?php if ($modShow && $modCat): ?>
		<div class="category-action text-center">
			<a class="btn btn-primary" href="<?php echo JRoute::_(
       "index.php?Itemid={$modMenu}"
   ); ?>" title="<?php echo $modCat; ?>">
					<?php echo $modCat; ?>
			</a>
		</div>
	<?php endif; ?>
